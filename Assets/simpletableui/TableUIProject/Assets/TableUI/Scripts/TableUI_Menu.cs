...
{
    public class TableUI_Menu
    {

        private const string kUILayerName = "UI";

        [MenuItem("GameObject/UI/Table")]
        static public void AddTableUI(MenuCommand menuCommand)
        {
            GameObject go = GenerateBaseObject();
                PlaceUIElementRoot(go, menuCommand);
                go.GetComponent<TableUI>().Init();
        }
        public static GameObject GenerateBaseObject()
        {
            //Generate main object
            GameObject table = new GameObject("Table");
            table.AddComponent<RectTransform>();
            table.AddComponent<TableUI>();
            ...
            table.AddComponent<ToggleProperties>().hideFlags = HideFlags.HideInInspector;
            table.GetComponents<ToggleProperties>()[0].Init();

            table.AddComponent<ToggleProperties>().hideFlags = HideFlags.HideInInspector;
            table.GetComponents<ToggleProperties>()[1].Init();

            table.AddComponent<ButtonProperties>().hideFlags = HideFlags.HideInInspector;
            table.GetComponents<ButtonProperties>()[0].Init();

            table.AddComponent<ButtonProperties>().hideFlags = HideFlags.HideInInspector;
            table.GetComponents<ButtonProperties>()[1].Init();

            table.AddComponent<DropdownProperties>().hideFlags = HideFlags.HideInInspector;
            table.GetComponents<DropdownProperties>()[0].Init();
            ...
            content.transform.SetParent(table.transform);
            ...
            borders.AddComponent<RectTransform>();
            borders.transform.SetParent(table.transform);
            borders.transform.localPosition = Vector3.zero;
            borders.AddComponent<UILineRenderer>().color = Color.black;
            borders.GetComponent<UILineRenderer>().raycastTarget = false;
            return table;
        }
